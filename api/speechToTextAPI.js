...
class SpeechToTextAPI {
    ...
    async setupWebSpeechAPI(config) {
        ...
        this.recognition.onend = () => {
            ...
            // Don't automatically restart - prevent errors
            if (this.onEndCallback) {
                this.onEndCallback();
            }
            
            // Only restart if explicitly requested and still active
            // Removed automatic restart to prevent abort errors
        };
    }
    ...
    async startRecognition(language = 'en-US') {
        try {
            ...
            if (this.recognition.lang !== language) {
                console.log(`🌍 Updating language from ${this.recognition.lang} to ${language}`);
